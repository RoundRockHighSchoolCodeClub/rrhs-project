...
class Player(Character):
  ...
  def explore(self):
    if self.state != 'normal':
     ...
     # self.enemy_attacks() Do not uncomment this. Not having an enemy loaded will cause this to err. 
    else:
      ...
      if randint(0, 1):
        ...
        if randint(0, 1): self.shop()
        else:
            if randint(0, 1): self.tired()
  def shop(self):
      self.state = 'buy'
      print ("%s finds a shop! They are selling health potions for 2 coins." % (self.name))
  def buy(self):
    if self.state != 'buy': print ("%s offers coins to the wind." % (self.name))
    else:
        print ("%s buys a health potion!" % (self.name))
        self.state = 'normal'
  def leave(self):
      if self.state != 'buy': print ("%s takes a singular step foward." % (self.name))
      else:
          self.state = 'normal'
          print ("%s leaves the shop wondering if they will ever be back." % (self.name))
  def flee(self):
